...
print("Generating HI maps at {} redshifts\n".format(len(redshifts)))
print("----------------------------------------------")
start=time.time()
...
for z in redshifts:
    ...
    nh *= nxion
    print("volume averaged neutral hydrgen fraction(for real-space data) = {:.4f}".format(np.average(nxion)))
    print("Mass averaged Neutral Hydrogen Fraction (for data in real space)= {:.4f}".format(np.average(nh)/robar))
    np.save("ionz_out/HI_map_{:.3f}.npy".format(z),nh)
    ...
    np.save("ionz_out/HI_maprs_{:.3f}.npy".format(z),nh)
    print("Mass averaged Neutral Hydrogen Fraction (for data in redshift space) = {:.4f}".format(np.average(nh)/robar))
    print("-------------------------------------------------")
